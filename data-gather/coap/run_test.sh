...
perun monitor data-gather/coap/coap_receiver.py --host localhost --port 5683 --resource test --count "$count" &
receive_pid=$!
sleep 5
perun monitor data-gather/coap/coap_sender.py --host localhost --port 5683 --resource test --rate "$rate" --payload-size 64 --concurrency "$conc" --count "$count" &
send_pid=$!
